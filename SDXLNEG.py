...
negative_prompt_str = 'blurry, deformed, ugly, lowres, bad faces and hands, text, missing fingers'
...
def run_worker(gpu_id, rows):
    ...
    for image_id, title in tqdm(rows, desc=f"GPU {gpu_id}", position=gpu_id):
        try:
            img_start = time.time()
            image = pipeline(prompt=title, generator=generator, negative_prompt=negative_prompt_str, height=1024, width=1024,num_inference_steps=40, guidance_scale=7.5).images[0]
